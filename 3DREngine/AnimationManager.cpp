...
CAnimationController *CAnimationManager::GetAnimationController( const char *sPath )
{
	...
	unsigned int uiSize;
	while (UTIL_Read( fAnimationController, &uiSize, 1, unsigned int ))
	{
		char *sAnimationName = new char[uiSize + 1];
		UTIL_Read( fAnimationController, sAnimationName, uiSize, char );
		sAnimationName[uiSize] = '\0';
